...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f"{SIMULATION_PATH}/{VERSION_1_ESFT}/{args.qa_model.shortname}"):
        os.makedirs(f"{SIMULATION_PATH}/{VERSION_1_ESFT}/{args.qa_model.shortname}")
    
    pulled_conversations = json.load(open(f"{SIMULATION_PATH}/{VERSION_1_ESFT}/{args.qa_model.shortname}/{args.split.name}.json", 'r'))
    ...
    if args.turn.number != args.MAX_TURNS:
        ...
        with open(f"{SIMULATION_PATH}/{VERSION_1_ESFT}/{args.qa_model.shortname}/{args.split.name}_turn-{args.turn.number}.json", 'w') as f:
