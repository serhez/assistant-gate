...
def main(args: DictConfig) -> None:
    ...
    if os.path.exists(f"{SIMULATION_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}/{args.split.name}.json"):
        ...
        for j, persona in enumerate(personas):
            ...
            for batch_index, conversation_batch in enumerate(conversation_batches):
                ...
                json_prompts = conversation_batch
                chat_formatted_initial_prompts = [
                    tokenizer.apply_chat_template(prompt, tokenize=False) for prompt in json_prompts
                ]
                qa_responses = qa_model.batch_prompt(chat_formatted_initial_prompts, **args.qa_model.run.completion_config)
                qa_responses = [sanitize_response(response) for response in qa_responses]
                appended_conversations = [unfinished_conversation + [{"role" : "assistant", "content" : qa_response}] for unfinished_conversation, qa_response in zip(json_prompts, qa_responses)]
        ...
        for j, persona in enumerate(personas):
            ...
            for batch_index, prompt_batch in enumerate(prompt_batches):
                ...
                json_prompts = [[{"role" : "user", "content" : QA_PROMPTS[args.QA_PROMPT_IDX].format(names[j], prompt)}] for prompt in prompt_batch]
                chat_formatted_initial_prompts = [
                    tokenizer.apply_chat_template(prompt, tokenize=False) for prompt in json_prompts
                ]
                
                # QA Model initial turn
                qa_responses = qa_model.batch_prompt(chat_formatted_initial_prompts, **args.qa_model.run.initial_completion_config)  # has length num_return_sequences * batch_size
                qa_responses = [sanitize_response(response) for response in qa_responses]
                conversations = list()
                for i, sublist in enumerate(chunk_list(qa_responses, args.qa_model.run.initial_completion_config.num_return_sequences)):
                    conversations.extend([json_prompts[i] + [{"role" : "assistant", "content" : qa_response}] for qa_response in sublist])
