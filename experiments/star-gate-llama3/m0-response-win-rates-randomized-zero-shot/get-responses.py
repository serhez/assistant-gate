...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{WINRATE_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}'):
        os.makedirs(f'{WINRATE_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}')
    ...
    for i in range(1, args.MAX_TURNS + 1):
        turns_conversations.append(json.load(open(f"{SIMULATION_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}/{args.split.name}_turn-{i}.json", "r")))
    pooled_conversations = json.load(open(f"{SIMULATION_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}/{args.split.name}{f'_top-k-{args.k}' if args.k > 0 else ''}.json", "r"))
    ...
    for t_num, group in enumerate([turns_1, turns_2, turns_3]):
        ...
        with open(f'{WINRATE_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}/{args.split.name}_turn-{t_num + 1}_m0-responses_zero_shot.json', 'w') as f:
