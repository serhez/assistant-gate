...
def main(args: DictConfig) -> None:
    ...
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_1.append(json.load(open(f'{WINRATE_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}/{args.split.name}_turn-{i}_responses_zero_shot.json', 'r')))
    
    qa_responses_2 = []
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_2.append(json.load(open(f'{WINRATE_PATH}/{LLAMA_VERSION}/{args.qa_model_2.shortname}/{args.split.name}_turn-{i}_responses_zero_shot.json', 'r')))
    ...
    for t_num, group in enumerate([turns_1, turns_2, turns_3]):
        ...
        if t_num == 2:
            ...
            if t_num == 2:
                with open(f'{WINRATE_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}_{args.qa_model_2.shortname}/{args.split.name}_turn-{t_num + 1}_win-rates-randomized_zero_shot.json', 'w') as f:
