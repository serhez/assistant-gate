...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{WINRATE_PATH}/{VERSION_3_QSFT}/{args.qa_model.shortname}_{args.qa_model_2.shortname}'):
        os.makedirs(f'{WINRATE_PATH}/{VERSION_3_QSFT}/{args.qa_model.shortname}_{args.qa_model_2.shortname}')
    
    
    # Load personas, prompts
    with open(f"{PERSONAS_PATH}/{VERSION_3_QSFT}/{args.split.name}.json", 'r') as f:
        personas = json.load(f)
    with open(f"{PERSONAS_PATH}/{VERSION_3_QSFT}/{args.split.name}_NAMES.json", 'r') as f:
        names = json.load(f)
    ...
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_1.append(json.load(open(f'{WINRATE_PATH}/{VERSION_3_QSFT}/{args.qa_model.shortname}/{args.split.name}_turn-{i}_responses.json', 'r')))
    
    qa_responses_2 = []
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_2.append(json.load(open(f'{WINRATE_PATH}/{VERSION_3_QSFT}/{args.qa_model_2.shortname}/{args.split.name}_turn-{i}_responses.json', 'r')))
    ...
    for t_num, group in enumerate([turns_1, turns_2, turns_3]):
        ...
        #flip, so m1, m2, m3 etc is always first and baseline is always second
        group_qa_responses_1, group_qa_responses_2 = group_qa_responses_2, group_qa_responses_1
        ...
        with open(f'{WINRATE_PATH}/{VERSION_3_QSFT}/{args.qa_model.shortname}_{args.qa_model_2.shortname}/{args.split.name}_turn-{t_num + 1}_win-rates-counterbalanced.json', 'w') as f:
