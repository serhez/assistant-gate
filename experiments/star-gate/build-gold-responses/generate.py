...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f"{GOLD_PATH}/{VERSION_2_BSFT}"):
        os.makedirs(f"{GOLD_PATH}/{VERSION_2_BSFT}")
    ...
    for j, persona in enumerate(personas):
        ...
        with open(f"{GOLD_PATH}/{VERSION_2_BSFT}/{args.split.name}.json", 'w') as f:
