...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f"{MODELRESPONSE_PATH}/{VERSION_2_BSFT}/{args.qa_model.shortname}"):
        os.makedirs(f"{MODELRESPONSE_PATH}/{VERSION_2_BSFT}/{args.qa_model.shortname}")
    
    with open(f"{MODELRESPONSE_PATH}/{VERSION_2_BSFT}/{args.qa_model.shortname}/{args.split.name}_top-k-{args.k}.json", 'w') as f:
