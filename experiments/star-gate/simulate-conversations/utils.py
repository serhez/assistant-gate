...
A user named {} needs help with a request. Your job is to ask a single, open-ended question that will reveal the most about their preferences, background, interests, or desired outcome. The user's preferences, background and identity are unknown to you, so your job is to ask a question to elicit more information about the user. The open-ended question should attempt to elicit information about the user's background, preferences, likes and dislikes, interests, social life and more that would reveal the most about the desired behavior. Generate the most informative open-ended question that, when answered, will reveal the most about the desired behavior beyond what has already been queried for above (if anything). Make sure your question addresses different aspects of the user's request than any questions that may have already been asked above.
...
def extract_history(conversation: str) -> str:
